...
{
public:
    static std::vector<double> getValsFromString(std::string town, const std::string &str);
public:
    static double mean(std::string town, const std::string &str);
    static double variance(std::string town, const std::string &str);
};

std::vector<double> Rainfall::getValsFromString(std::string town, const std::string &str)
{
    std::string token;
    std::stringstream ss(str);
    while(getline(ss, token, '\n'))
    {
        if(token.find(town) != std::string::npos)
        {
            break;
        }
    }
    ...
    if(token[token.find(town) + town.length()] != ':')
    {
        return vals;
    }
    while(idx != std::string::npos)
    {
        vals.push_back(std::stod(token.substr(idx)));
...
{
    std::vector<double> vals = getValsFromString(town, str);
    if(vals.size() == 0){
        return -1;
    }
    double sum(0);
...
void assertFuzzyEquals(double act, double exp)
{
    ...
    if (inrange == false)
    {
        ...
        std::cout << "Testing at 1e-2; Expected should be " << exp << ", but got " << act << std::endl;
    }
    assert(true == inrange);
}
...
int main()
{
    std:: cout << "Fixed Tests: mean data" << std::endl;
    ...
    std:: cout << "Fixed Tests: variance data" << std::endl;
